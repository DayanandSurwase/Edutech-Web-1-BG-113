...
function Button(variant, classes, attrs, innerHTML, disabled) {
  const base =
    "inline-flex items-center justify-center gap-2 rounded-xl font-medium transition-all duration-200 cursor-pointer select-none btn-transition";
  const variants = {
    primary:
      "bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg shadow-indigo-500/25 px-5 py-2.5",
    secondary:
      "bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 px-5 py-2.5",
    ghost:
      "hover:bg-slate-100 dark:hover:bg-slate-700/60 text-slate-600 dark:text-slate-300 px-4 py-2",
    danger: "bg-red-500 hover:bg-red-400 text-white px-5 py-2.5",
    outline:
      "border border-indigo-500 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 px-5 py-2.5",
    gradient:
      "bg-gradient-to-r from-indigo-600 to-cyan-500 hover:from-indigo-500 hover:to-cyan-400 text-white shadow-lg shadow-indigo-500/30 px-5 py-2.5",
  };
...
function renderDashboard() {
  ...
  let skillsHtml = recommendedSkills
    .map(
      (s) => `
        ${Card(
...
function renderProgress() {
  const progs = Object.entries(state.progress)
    .map(([sub, val]) => {
      const color =
        sub === "Math"
          ? "#4F46E5"
  ...
  const sliders = Object.entries(state.progress)
    .map(([sub, val]) => {
      const color =
        sub === "Math"
          ? "#4F46E5"
      ...
      return `
            <div>
              ...
              <input type="range" min="0" max="100" value="${val}" oninput="state.progress['${sub}']=parseInt(this.value); render();" class="w-full h-2 rounded-full appearance-none cursor-pointer" style="accent-color: ${color}">
            </div>
         `;
    })
    .join("");
...
function renderGoals() {
  ...
  const pct = state.goals.length
    ? Math.round((doneCount / state.goals.length) * 100)
    : 0;

  let goalsHtml = state.goals
    .map(
      (goal, i) => `
  ...
  if (state.goals.length === 0)
    goalsHtml = `<div class="p-12 text-center"><i data-lucide="target" class="w-10 h-10 text-slate-300 dark:text-slate-600 mx-auto mb-3"></i><p class="text-slate-500 dark:text-slate-400 font-medium">No goals yet</p></div>`;
...
// ADD session (REAL API)
async function addSession() {
  try {
    if (!state.plannerForm.date) {
      return toast("Pick a date", "error");
    }
...
function renderPlanner() {
  let sessionList = state.sessions
    .map(
      (s) => `
        ${Card(
          ...
          `
            ...
            <div class="flex-1">
              <p class="text-sm font-medium text-slate-800 dark:text-white">${s.subject}</p>
              <p class="text-xs text-slate-400">${s.date}</p>
            </div>

            <div class="flex items-center gap-2">
              <div class="flex items-center gap-1 text-slate-500">
                <i data-lucide="clock" class="w-3 h-3"></i>
                <span class="text-xs">${s.duration}m</span>
              </div>

              <button onclick="deleteSession('${s.id}')" class="text-red-400 hover:text-red-600 text-xs">
                Delete
              </button>
            </div>
    ...
    modalHtml = `
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="absolute inset-0 bg-black/60 backdrop-blur-sm"
             onclick="state.plannerForm.modalOpen=false; render()"></div>

        <div class="relative z-10 w-full max-w-md rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-2xl p-6">

          <h3 class="mb-4 text-lg font-semibold">Add Study Session</h3>

          <div class="space-y-4">

            <input type="date"
              oninput="state.plannerForm.date = this.value"
              value="${state.plannerForm.date}"
              class="w-full px-4 py-2.5 rounded-xl border bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white">

            <select
              onchange="state.plannerForm.subject = this.value"
              class="w-full px-4 py-2.5 rounded-xl border bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white">
            ...
            <input type="number"
              ...
              oninput="state.plannerForm.duration = this.value"
              value="${state.plannerForm.duration}"
              class="w-full px-4 py-2.5 rounded-xl border bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white">
            ...
            <div class="flex gap-3">
              ${Button(
                "secondary",
                "flex-1",
                'onclick="state.plannerForm.modalOpen=false; render()"',
                "Cancel"
              )}
  ...
  return `
    <div class="space-y-6 fade-up">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-lg font-bold text-slate-800 dark:text-white">Study Planner</h2>
          <p class="text-sm text-slate-500">Dynamic Sessions</p>
        </div>

        ${Button(
          "gradient",
          "",
          'onclick="state.plannerForm.modalOpen=true; render()"',
...
// Navigate to a section; if planner, also reload sessions from backend
window.navigateTo = (sectionId, closeMobile = false) => {
  state.activeSection = sectionId;
  if (closeMobile) state.mobileOpen = false;
  render();
...
window.submitPractice = () => {
  ...
  toast("Evaluation needs backend", "info");
  render();
};
...
function renderProfile() {
  ...
  if (state.profileForm.editing) {
    const intList = allInterests
      .map(
        (i) =>
          `<button onclick="toggleProfileInterest('${i}')" class="px-3 py-1 rounded-full text-xs font-medium ${u.interests.includes(i) ? "bg-indigo-600 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500"}">${i}</button>`,
      )
      .join("");
    const lvlList = skillLevels
      .map(
        (l) =>
          `<button onclick="state.profileForm.skillLevel='${l}';render()" class="py-2 rounded-xl text-xs font-medium ${u.skillLevel === l ? "bg-indigo-600 text-white" : "bg-slate-100 dark:bg-slate-800"}">${l}</button>`,
      )
      .join("");
    mainContent = `
            <div class="space-y-4">
        ...
        ${Card(
          ...
          `
            <div class="flex items-start justify-between mb-5">
                ...
                ${
                  !state.profileForm.editing
                    ? Button(
                        "ghost",
                        "",
                        'onclick="editProfile()"',
                        '<i data-lucide="edit-2" class="w-3"></i> Edit',
                      )
                    : `
...
window.toggleProfileInterest = (i) => {
  if (state.profileForm.interests.includes(i))
    state.profileForm.interests = state.profileForm.interests.filter(
      (x) => x !== i,
    );
...
function renderSidebar(isMobile = false) {
  ...
  const itemsHtml = navItems
    .map((item) => {
      ...
      return `
            <button onclick="navigateTo('${item.id}', ${isMobile})" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer ${classes}">
...
window.handleLogin = async (e) => {
  ...
  try {
    ...
    state.user.email = state.loginForm.email;
    state.page = "app";
